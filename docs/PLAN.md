...
| 2.5 | ~~**Region of interest per video source**~~ | Justin | **Done 2026-09-06.** `ingest/collection/view_region.py` and `calibrate_region.py`, honoured by the C++ analyzer via `regions_path`. **35 of 60 sources** composite a second view, seams from 0.44 to 0.77 — so no single crop line serves. The seam is not readable from one frame (row-discontinuity scores 4.55 on stacked frames against 4.49 on flat ones, i.e. chance), so it is calibrated per source from where robots are found. Filtering after inference beat cropping before it, 4.22 boxes per frame against 4.05, because the model was trained on whole frames. |
